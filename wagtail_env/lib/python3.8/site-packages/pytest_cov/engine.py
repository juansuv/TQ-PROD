...
class DistMain(CovController):
    ...
    def configure_node(self, node):
        ...
        workerinput(node).update({
            'cov_main_host': socket.gethostname(),
            'cov_main_topdir': self.topdir,
            'cov_main_rsync_roots': [str(root) for root in node.nodemanager.roots],
        })
...
class DistWorker(CovController):
    ...
    def start(self):
        ...
        self.is_collocated = (socket.gethostname() == workerinput(self.config)['cov_main_host'] and
                              self.topdir == workerinput(self.config)['cov_main_topdir'])

        # If we are not collocated then rewrite main paths to worker paths.
        if not self.is_collocated:
            main_topdir = workerinput(self.config)['cov_main_topdir']
            worker_topdir = self.topdir
            if self.cov_source is not None:
                self.cov_source = [source.replace(main_topdir, worker_topdir)
                                   for source in self.cov_source]
            self.cov_config = self.cov_config.replace(main_topdir, worker_topdir)
